package Silent
{
	...
	public class PlayState extends FlxState
	{
		...
		private var _packsText:FlxText = new FlxText(32, 0, 300, "x0"); // texto de quantos packs o player tem (Canto superior dir)
		private var _hudPack:FlxSprite = new FlxSprite(0, 0); // imagem que fica ao lado o numero de packs
		//private const _vinheta:FlxSprite = new FlxSprite(0, 0); // sprite de efeito de vinheta
		
		private var _savePackLayer:FlxGroup = new FlxGroup(); // layer aonde é colocado todos os save packs para se fazer teste de colisao
		...
		private var _saveStateLayer:FlxGroup = new FlxGroup();
		private var _mapLayer:FlxGroup = new FlxGroup();
		private var _creditsLayer:FlxGroup = new FlxGroup();
		
		private const _array:Array = [_savePackLayer, _cameraHolderLayer, _checkPointLayer, _saveStateLayer, _mapLayer, _creditsLayer];
		...
		override public function create() : void 
		{
			...
			_hudPack.loadGraphic(Assets.ImgHudPack, false, false);
		...
		private function loadLanguage():void {
			...
			posArray = [ { x:900, y:900, text:LanguagePack.PRESS_JUMP[language], stage:0, blend:blendInstru, textSize:35, color:0xffffff,alpha:alphaInstru },
						...
						{ x:3520, y:672 , text:LanguagePack.COLLECT[language], stage:2, blend:blendInstru, textSize:35, color:0xffffff,alpha:alphaInstru },
						{ x:6464, y:480 , text:LanguagePack.TAKE_PICTURE[language], stage:2, blend:blendInstru, textSize:35, color:0xffffff,alpha:alphaInstru },
						{ x:1472, y:928 , text:LanguagePack.WALL_JUMP[language], stage:3, blend:blendInstru, textSize:35, color:0xffffff,alpha:alphaInstru },
						{ x:9600, y:576 , text:LanguagePack.JOE_WHY_1[FlxG.language], stage:0, blend:blendTexts, textSize:25, color:0x4f1401, alpha:1 },
						{ x:8192, y:704 , text:LanguagePack.JOE_SOMEONE_2[FlxG.language], stage:1, blend:blendTexts, textSize:25, color:0x4f1401, alpha:1 },
						{ x:8768, y:320 , text:LanguagePack.JOE_BUT_WHAT_3[FlxG.language], stage:2, blend:blendTexts, textSize:25, color:0x4f1401, alpha:1 },
						{ x:2304, y:736 , text:LanguagePack.JOE_AND_WHAT_4[FlxG.language], stage:3, blend:blendTexts, textSize:25, color:0x4f1401, alpha:1 },
						{ x:17600, y:864 , text:LanguagePack.JOE_OH_SORRY_5[FlxG.language], stage:4, blend:blendTexts, textSize:25, color:0x4f1401, alpha:1 },
						{ x:8768, y:608 , text:LanguagePack.JOE_THERE_STILL_7[FlxG.language], stage:6, blend:blendTexts, textSize:25, color:0x4f1401, alpha:1 },
						{ x:11008, y:640 , text:LanguagePack.JOE_HAVE_8[FlxG.language], stage:7, blend:blendTexts, textSize:25, color:0x4f1401, alpha:1 },
						{ x:16576, y:192 , text:LanguagePack.JOE_WELL_VICTORY_9[FlxG.language], stage:8, blend:blendTexts, textSize:25, color:0x4f1401, alpha:1 } ];
		...
		public function moveCamera():void {
			...
			if (_runner.x < _autoCamera.x - 600) {
				reduceScore(150);
				load();
			}
		...
		public function checkDeath():void {
			if (_runner.y > _mapa.height) {
				Log.CustomMetric("X: " + String(Math.round(_runner.x)) + " Y: " + String(Math.round(_runner.y)), " DeathLevel: " + String(_mapNumber));
				Log.LevelCounterMetric("NumberOfDeaths", _mapNumber);
				gameTracker.customMsg("Death", score);
				countDeath++;
				load();
				reduceScore(150);
			}
		...
		public function checkModular():void {
			...
			if (_runner.x + 36 > _mapa.width && _runner.y < _mapa.height - 200  && _mapNumber != 8) {
				Log.LevelAverageMetric("Average Deaths", _mapNumber, countDeath);
				Log.LevelAverageMetric("Average Saves", _mapNumber, countSave);
				Log.LevelAverageMetric("Average Time", _mapNumber, countSecsStage);
				Log.LevelAverageMetric("Average Total Time", _mapNumber, countSecsTotal);
				Log.CustomMetric("Score: " + String(score), "ScoreLevel: " + String(_mapNumber));
				Log.LevelCounterMetric("Finished", _mapNumber);
				...
				_autoCamera.velocity.x = RunnerState.RUNNER_ASPEEDBOOST * 0.75;
				waitingCamera = false;
				gameTracker.endLevel(score,"Level: " + String(_mapNumber));
				...
				FlxG.fade.start(0xff000000, 3, startEnd);
				gameTracker.endGame(score);
			}
		}
		...
		public function createObjects():void {
			...
			for (var i:int = 0; i < posArray.length; ++i) {
				...
				_newObj.fixed = true;
				_newObj.size = 30;//posArray[i].textSize;
				_newObj.blend = posArray[i].blend;
				_newObj.color = posArray[i].color;
				_newObj.alpha = posArray[i].alpha;
				switch(FlxG.language) {
					case LanguagePack.ARABIC:
						_newObj.font = LanguagePack.fontArabic;
					break;
					
					case LanguagePack.JAPANESE:
						_newObj.font = LanguagePack.fontJapanese;
					break;
				}
			...
			for (i = 0; i < stageItens[_mapNumber].length; ++i) {
				switch(stageItens[_mapNumber][i].obj) {
					...
					case 25: // caso seja um savePack coloca no palco e coloca no layer para fazer testes de colisao
						...
						_newSaveObj.loadGraphic(Assets.ImgSavePack, false, false);
		...
		public function createSaveImage(stateName:String):FlxSprite {
			...
			switch(stateName) {
				case "running":
					_newImg.loadGraphic(Assets.ImgSaveRun, false, false, 72, 72, false);
				break;
				...
				case "dbl_descending":
					_newImg.x -= 13;
					_newImg.loadGraphic(Assets.ImgSaveJump, false, _runner.facing!=FlxSprite.RIGHT, 72, 72, false);
				break;
				
				case "glide_loop":
				case "glide":
					_newImg.x -= 16;
					_newImg.loadGraphic(Assets.ImgSaveGlide, false, _runner.facing!=FlxSprite.RIGHT, 72, 72, false);
				break;
				
				case "stood":
					_newImg.x -= 32;
					_newImg.y -= 4;
					_newImg.loadGraphic(Assets.ImgSaveStood, false, _runner.facing!=FlxSprite.RIGHT, 72, 72, false);
				break;
				...
				if(_runner.facing == FlxSprite.RIGHT){
					_newImg.x -= 3;
					_newImg.loadGraphic(Assets.ImgSaveWall, false, false, 72, 72, false);
				}else {
					_newImg.x -= 28;
					_newImg.loadGraphic(Assets.ImgSaveWall, false, true, 72, 72, false);
				}
				break;
			}
		...
		public function load():void {
			...
			if (flagsArray.length == 0) {
				...
				FlxG.play(Assets.SndRespawn, 1); // toca o som
				...
				if (flagsArray[flagsArray.length - 1].flagsLife == 0) {
					...
					flagsArray.pop(); // e deleta ele da array
				}
			}
		}
	}
